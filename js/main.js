...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const timer = setInterval(() => {
      if (navMusicEl && navMusicEl.querySelector("#nav-music meting-js").aplayer) {
        ...
        navMusicEl.querySelector("#nav-music meting-js").aplayer.on("pause", function () {
          ...
          document.getElementById("nav-music-hoverTips").innerHTML = "Music Paused";
  ...
  window.onkeydown = function (e) {
    123 === e.keyCode && anzhiyu.snackbarShow("Developer mode activated. Please comply with the GPL license.", !1);
  };
  ...
  function statistics51aInit() {
    ...
    Promise.all(scriptUrls.map(({ url, charset, crossorigin, id }) => loadScript(url, charset, crossorigin, id)))
      ...
      .catch(error => {
        console.error("Loading 51a analytics failed, but getting a 403 locally is expected behavior:", error);
      });
  }
